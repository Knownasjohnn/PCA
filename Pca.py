...
def check(myip, prox, url):
	try:
		...
		if ipx == myip:
			...
			print(Fore.GREEN + "Attacking website >>>> ".format(prox)+Style.RESET_ALL)
	...
	try:
		...
		if req1.status_code == 200:
			if "<title>Just a moment...</title>" in req1.text:
				...
				print(Fore.GREEN+"[+] Attacking website >>>".format(prox))
	except:
		pass
...
/_/     \____/  /_/  |_|
[+] Dev: Ph.Sk1d
[+] Philippines Cyber Alliance
	
""")
		
pwd = input(Fore.BLUE + "[+] Enter password: ")
...
if pwd == 'pca':
	...
	def main(proxy, url):
		...
		if url == None:
			print(Fore.RED + "")
			url = input(Fore.BLUE + "[+] Target:  ")
		if url[:4] != "http":
			print(Fore.BLUE+"Enter the full URL (example: https://google.com/)"+Style.RESET_ALL)
			exit()
		if proxy == None:
			while True:
				...
				array = req.split()
				print(Fore.GREEN+"Found {} new proxies".format(len(array))+Style.RESET_ALL)
	...
	print(Fore.RED + "Your not an PCA member!")
	os.system('exit')
